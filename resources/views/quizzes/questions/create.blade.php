    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-bold text-2xl text-white leading-tight flex items-center gap-3">
                ...
                Tambah Soal untuk Kuis: {{ $quiz->title }}
            </h2>
        </div>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-gray-900 overflow-hidden shadow-xl sm:rounded-2xl border border-gray-800">
                <div class="p-8" x-data="{ questionType: '{{ $lastType ?? 'multiple_choice' }}' }">
                    <form action="{{ route('quizzes.questions.store', $quiz) }}" method="POST" x-data="{ 
                        ...
                        <div class="mb-6">
                            ...
                            <textarea name="question_text" id="question_text" rows="4" class="w-full bg-gray-800 border border-gray-700 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-fuchsia-500 focus:border-fuchsia-500 transition-all" required placeholder="Tuliskan pertanyaan di sini..."></textarea>
                        </div>
                        ...
                        <div x-show="questionType === 'multiple_choice'" x-transition>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                                <div>
                                    ...
                                    <input type="text" name="option_a" id="option_a" class="w-full bg-gray-800 border border-gray-700 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-fuchsia-500 focus:border-fuchsia-500 transition-all" :required="questionType === 'multiple_choice'">
                                </div>
                                <div>
                                    <label for="option_b" class="block text-sm font-bold text-gray-300 mb-2">Opsi B</label>
                                    <input type="text" name="option_b" id="option_b" class="w-full bg-gray-800 border border-gray-700 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-fuchsia-500 focus:border-fuchsia-500 transition-all" :required="questionType === 'multiple_choice'">
                                </div>
                                <div>
                                    <label for="option_c" class="block text-sm font-bold text-gray-300 mb-2">Opsi C</label>
                                    <input type="text" name="option_c" id="option_c" class="w-full bg-gray-800 border border-gray-700 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-fuchsia-500 focus:border-fuchsia-500 transition-all" :required="questionType === 'multiple_choice'">
                                </div>
                                <div>
                                    <label for="option_d" class="block text-sm font-bold text-gray-300 mb-2">Opsi D</label>
                                    <input type="text" name="option_d" id="option_d" class="w-full bg-gray-800 border border-gray-700 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-fuchsia-500 focus:border-fuchsia-500 transition-all" :required="questionType === 'multiple_choice'">
                                </div>
                            </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                                <div>
                                    ...
                                    <select name="correct_option" id="correct_option" class="w-full bg-gray-800 border border-gray-700 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-all appearance-none cursor-pointer">
                                        <option value="A" class="bg-gray-800 text-white">A</option>
                                        <option value="B" class="bg-gray-800 text-white">B</option>
                                        <option value="C" class="bg-gray-800 text-white">C</option>
                                        <option value="D" class="bg-gray-800 text-white">D</option>
                                    </select>
                                </div>
                                <div>
                                    <label for="timer_seconds_pg" class="block text-sm font-bold text-gray-300 mb-2">Waktu Mengerjakan (Detik)</label>
                                    <input type="number" name="timer_seconds" id="timer_seconds_pg" value="30" min="5" class="w-full bg-gray-800 border border-gray-700 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-fuchsia-500 focus:border-fuchsia-500 transition-all" :required="questionType === 'multiple_choice'" :disabled="questionType !== 'multiple_choice'">
                                </div>
                            </div>
                        </div>
                        ...
                        <div x-show="questionType === 'essay'" x-transition>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                                <div>
                                    ...
                                    <input type="text" name="essay_answer" id="essay_answer" class="w-full bg-gray-800 border border-gray-700 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 transition-all" :required="questionType === 'essay'" placeholder="Contoh: Pancasila">
                                </div>
                                <div>
                                    <label for="timer_seconds_essay" class="block text-sm font-bold text-gray-300 mb-2">Waktu Mengerjakan (Detik)</label>
                                    <input type="number" name="timer_seconds" id="timer_seconds_essay" value="30" min="5" class="w-full bg-gray-800 border border-gray-700 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-fuchsia-500 focus:border-fuchsia-500 transition-all" :required="questionType === 'essay'" :disabled="questionType !== 'essay'">
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center justify-end gap-4 pt-4 border-t border-gray-800">
                            ...
                            <button type="submit" x-bind:disabled="isSubmitting" x-bind:class="{ 'opacity-50 cursor-not-allowed': isSubmitting }" class="px-6 py-3 rounded-xl bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-400 hover:to-emerald-500 text-white font-bold shadow-lg shadow-green-500/30 transition-all">
                                <span x-show="!isSubmitting">Simpan Soal</span>
